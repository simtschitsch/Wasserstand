(function ( $ ){
	...
	*/
 	...
 	var defaultKey		= 'bcVXnxDC9iQC0iqfQ90ChPLgh3nQZFIgrXbwPFfw37VS500D', // Unique master Xively API key to be used as a default
		defaultFeeds	= [85178], // Comma separated array of Xively Feed ID numbers
		...
		xively.feed.get(feedId, function(feedData) {
			if(feedData.datastreams) {
				...
				feedData.datastreams.forEach(function(datastream) {
					...
					then.setTime(utcnow.getTime() - diff);
							...
							xively.datastream.history(feedId, datastream.id, {duration: duration, interval: interval, limit: 1000}, function(datastreamData) {
								...
								if(datastreamData.datapoints) {
									...
									datastreamData.datapoints.forEach(function(datapoint) {
									 	points.push({x: new Date(datapoint.at).getTime()/1000.0, y: parseFloat(434-datapoint.value)});
									});
